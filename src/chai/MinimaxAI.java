...
public class MinimaxAI implements ChessAI {
	...
	// Constants
	private static int MAX_UTILITY = Integer.MAX_VALUE;
	private static int MIN_UTILITY = Integer.MIN_VALUE;
	...
	private int minValue(Position position, int depth) {
		incrementVisited();
	...
	private int maxValue(Position position, int depth) {
		incrementVisited();
		incrementDepth(depth);
	...
	private int evalFn(Position position) {
		// Return a random int between MIN_UTILITY and MAX_UTILITY

		// return ((MIN_UTILITY + 1) + (int)(Math.random() * ((MAX_UTILITY - MIN_UTILITY - 1))));
